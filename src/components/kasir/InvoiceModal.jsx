import { X, CheckCircle, Printer, Calendar, User, CreditCard, Hash } from 'lucide-react';
...
export default function InvoiceModal({ transaction, paymentResult, onClose }) {
  ...
  const orderId = paymentResult?.order_id || transaction.order_id || (transaction.id ? String(transaction.id).slice(0, 12).toUpperCase() : 'UNNAMED');
  ...
  const handlePrint = () => {
    ...
    const method = (() => {
      ...
      return m.replace(/_/g, ' ') || 'Midtrans Gateway';
    })();
            ...
            <div className="flex flex-col md:flex-row justify-between gap-6 mb-10">
              ...
              <div className="md:text-right">
                ...
                <div className="flex flex-col gap-1 items-start md:items-end">
                  ...
                  <p className="text-[11px] text-gray-400 font-medium uppercase tracking-wider">Status: <span className={`font-black ml-1 ${String(transaction.status).toLowerCase() === 'paid' ? 'text-green-500' : 'text-orange-500'}`}>{String(transaction.status || 'PENDING').toUpperCase()}</span></p>
                </div>
              </div>
            </div>
